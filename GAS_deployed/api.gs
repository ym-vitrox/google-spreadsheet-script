...
/**
 * Generate printable HTML for PDF creation
 * Returns HTML content that can be opened in a new window for printing
 *
...
function generatePDF(pdfData) {
  try {
    console.log('Server: Generating printable HTML with data:', pdfData);

    // Create HTML content for printing to PDF
    const htmlContent = createPDFHTML(pdfData);

    console.log('Server: Printable HTML generated successfully');
    return htmlContent;
 ...
 */
function createPDFHTML(pdfData) {
  const timestamp = new Date(pdfData.timestamp);
  const formattedDate = Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'dd/MM/yyyy HH:mm');
...
<head>
    ...
    <script>
        // Show print instructions when page loads
        window.onload = function() {
            // Don't auto-print anymore - let user manually print
            console.log('PDF-ready page loaded. Use Ctrl+P to save as PDF.');
        };

        // Add keyboard shortcut hint
        document.addEventListener('keydown', function(e) {
            if ((e.ctrlKey || e.metaKey) && e.key === 'p') {
                console.log('Print dialog opened - select "Save as PDF"');
            }
        });
    </script>
</head>
<body>
    <div class="header">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <div>
                <h1>BOM Order List</h1>
                <p>Bill of Materials Configuration</p>
            </div>
            <div style="text-align: right;">
                <p>Generated: ${formattedDate}</p>
                <p>Total Items: ${pdfData.totalItems} | Checked: ${pdfData.checkedItems} | Unchecked: ${pdfData.uncheckedItems}</p>
            </div>
        </div>
    </div>
    ...
    <div class="stats">
        <div class="stat-item">
            ...
            <span class="stat-value">${pdfData.sections.CORE} item${pdfData.sections.CORE !== 1 ? 's' : ''}</span>
        </div>
        <div class="stat-item">
            <span class="stat-label">⚙️ CONFIG:</span>
            <span class="stat-value">${pdfData.sections.CONFIG} item${pdfData.sections.CONFIG !== 1 ? 's' : ''}</span>
        </div>
        <div class="stat-item">
            <span class="stat-label">🔧 MODULE:</span>
            <span class="stat-value">${pdfData.sections.MODULE} item${pdfData.sections.MODULE !== 1 ? 's' : ''}</span>
        </div>
        <div class="stat-item">
            <span class="stat-label">👁 VISION:</span>
            <span class="stat-value">${pdfData.sections.VISION} item${pdfData.sections.VISION !== 1 ? 's' : ''}</span>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 8%; text-align: center;">Checked</th>
                <th style="width: 12%; text-align: center;">Date</th>
                <th style="width: 12%; text-align: center;">Release Type</th>
            </tr>
        </thead>
        <tbody>
`;
  ...
  // Add table rows
  let currentSection = '';
  pdfData.orderList.forEach(function(item) {
    // Section header
    if (item.section !== currentSection) {
      currentSection = item.section;
      html += `
            <tr class="section-header">
                <td colspan="7">${getSectionIcon(item.section)} ${item.section}</td>
            </tr>
`;
    }
    ...
    html += `
            <tr class="${rowClass}">
                ...
                <td style="${indent}font-family: monospace;">${depthMarker}${item.partId}</td>
                <td style="color: #475569;">${item.description}</td>
                <td style="text-align: center; color: #64748b;">${item.quantity}</td>
                <td style="text-align: center; color: ${checkedColor}; font-weight: bold;">${checkedIcon}</td>
                <td style="text-align: center; color: #64748b;">${item.checkDate || '—'}</td>
                <td style="text-align: center; color: #64748b; font-size: 8px;">${item.releaseType || '—'}</td>
            </tr>
`;
  });
  ...
  html += `
    ...
    <div class="footer">
        <p>BOM Configurator System | Export Date: ${formattedDate}</p>
    </div>

    <div class="no-print" style="position: fixed; top: 10px; right: 10px; background: #1e293b; color: white; padding: 10px; border-radius: 4px; z-index: 1000;">
        <p style="margin: 0; font-size: 14px; font-weight: bold;">
            📄 Save as PDF: Press Ctrl+P (or Cmd+P on Mac)
        </p>
    ...
    <div class="no-print" style="text-align: center; margin-top: 20px; color: #64748b; font-size: 12px; padding: 10px; background: #f8fafc; border-radius: 4px;">
        <p style="margin: 0;">
            💡 <strong>Tip:</strong> This page is optimized for printing to PDF.<br>
            The content above will be saved to your PDF file.
        </p>
    </div>
</body>
</html>`;
